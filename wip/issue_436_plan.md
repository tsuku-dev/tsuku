...
- [ ] Create `decomposable.go` with `Decomposable` interface definition
- [ ] Add `Step` struct with Action, Params, Checksum, Size fields
- [ ] Add `EvalContext` struct with Version, VersionTag, OS, Arch, Recipe, Resolver fields
- [ ] Add primitive registry (set of Tier 1 primitive names)
- [ ] Implement `IsPrimitive(action string) bool` function
- [ ] Create unit tests for `IsPrimitive` function
...
## Success Criteria
- [ ] `Decomposable` interface defined with `Decompose(ctx *EvalContext, params map[string]interface{}) ([]Step, error)` method
- [ ] `Step` struct has Action, Params, Checksum, Size fields
- [ ] `EvalContext` struct has Version, VersionTag, OS, Arch, Recipe, Resolver fields
- [ ] `IsPrimitive("download")` returns true
- [ ] `IsPrimitive("github_archive")` returns false
- [ ] All 8 Tier 1 primitives registered: download, extract, chmod, install_binaries, set_env, set_rpath, link_dependencies, install_libraries
- [ ] Unit tests pass
- [ ] Build succeeds
